...
public class AlunoControllerTest {
	...
	@Mock
	private ClienteApiCep clientApiCep;
	...
	public void deveAtualizaEnderecoAPartirDePesquisaDeCep() {
		
		TipoLogradouro rua = TipoLogradouro.builder().withAbreviatura("R").withDescricao("Rua").build();
		TipoLogradouro ruaIntegracao = TipoLogradouro.builder().withAbreviatura("R I").withDescricao("Rua Integração").build();

		Municipio abadiaGoias = Municipio.builder().withCodigoIbge(5200050).withNome("ABADIA DE GOIÁS").withUf(EnumUnidadeFederacao.GO).build();
		Municipio catalao = Municipio.builder().withCodigoIbge(5205109).withNome("CATALÃO").withUf(EnumUnidadeFederacao.GO).build();
		
		Estado goias = Estado.builder().withCodigo((short)52).withNome("Goiás").withUf(EnumUnidadeFederacao.GO).withMunicipios(List.of(abadiaGoias, catalao)).build();

		EnderecoApiCep enderecoApi = EnderecoApiCep
				.builder()
				.withBairro("Residencial Portal do Lago I")
				.withCep("75709-773")
				.withCidade("Catalão")
				.withUf("GO")
				.withLogradouro("Rua Integração 29")
				.withIbge("5205109")
				.build();
		...
		when(clientApiCep.obtemEndereco("75709-773")).thenReturn(enderecoApi);
		...
		verify(clientApiCep, times(1)).obtemEndereco("75709-773");
