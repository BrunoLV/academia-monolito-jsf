		...
		renderizaGraficoEvolucao : function(idAluno) {
			...
			function drawChart() {
				...
				$.get('http://localhost:8080/academia/services/aluno/'
					...
					dadosGrafico.push([ 'Data Medição', 'Peso', 'Pescoço',
							'Tórax Superior', 'Tórax Inferior', 'Busto',
							'Cintura', 'Abdômen', 'Quadril', 'Coxa Direita',
							'Coxa Esquerda', 'Panturrilha Direita',
							'Panturrilha Esquerda', 'Braço Direito',
							'Braço Esquerdo', 'Antebraço Direito',
							'Antebraço Esquerdo' ]);

					var i;
					for (i = 0; i < medidas.length; i++) {
						dadosGrafico.push([ medidas[i].dataMedicao,
								medidas[i].peso, medidas[i].pescoco,
								medidas[i].toraxSuperior,
								medidas[i].toraxInferior, medidas[i].busto,
								medidas[i].cintura, medidas[i].abdomen,
								medidas[i].quadril, medidas[i].coxaDireita,
